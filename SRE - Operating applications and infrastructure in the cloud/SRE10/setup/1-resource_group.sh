...
echo "Creating resource group for KeyVault $KEYVAULT_RG in $LOCATION"
az group create -l $LOCATION -n $KEYVAULT_RG -o table &>> $base_source_path/../setup/log/1-resource_group.log
echo ""

echo "Creating resource group for AppInsights $INSIGHTS_RG in $LOCATION"
az group create -l $LOCATION -n $INSIGHTS_RG -o table &>> $base_source_path/../setup/log/1-resource_group.log
echo ""
